...
- This project is a continuation of a similar project whose development was interrupted due to family circumstances.  
 ...
 **Home Page**
 ...
 - **a.** [Form Section LG](./docs/images/form%20index%20page.png);  [Form Section MD](./docs/images/form%20index%20page%20md.png); [Form Section SM](./docs/images/form%20index%20page%20sm.png) makes it possible for the logged user to enter the new book entry.
 ...
 The logged-in user has visible [edit and delete buttons](./docs/images/form%20index%20page%202%20edit%20and%20delete.png), whose function is explained below: 
 ...
 The logged-in user has the option to delete it's contribution. The custom [modal](./docs/images/Test%20delete%20book.png) prevents non-intentional deletion.
...
Upon editing [edit 1](./docs/images/edit%20bibliography%20message.png) [edit 2 with message](./docs/images/edit%20bibliography%20message%201.png) [edit 3](./docs/images/edit%20bibliography%20message%202.png) or deleting [delete 1](./docs/images/delete%20book.png) [delete 2](./docs/images/delete%20book%201.png) [delete 3 with message](./docs/images/delete%20book%202.png) bibliographical units, the appropriate messageses popup. 
 ...
 - **a.** Book Description, containing Title, Author(s), Publishing Year, Publisher, "Created by" and "Created on" fields, as well as the Book Summary fields. Only the [LG](./docs/images/individual%20book%20description%20LG.png) format contains the book image, while this feature is missing in the [MD](./docs/images/individual%20book%20description%20MD.png) and [SM](./docs/images/individual%20book%20description%20SM.png) formats to make content navigation easier for users.
 ...
 **About page**
 ...
 - a: two short textual explanations about the purpose of the project. One of them contains the link to the *Home* page to make the navigation easier, as illustrated in the capture "About LG", below. 
...
- *About page* 1: About link in the navbar. 
- *Home page* and *About page* available in the so-called ["hamburger menu"](docs/images/links%20responsive.png) for the small-size screens: 
- *List of book navigation* - Next and Prev buttons introduced [Next Only](static/images/next_only.png); [Prev Next](static/images/prev_next.png); 
...
## Messages
In order to improve the UI, the appropriate messages emerge on the screen following the user's **CRUD OPERATIONS**. A small button allows the user to delete popup messages.
...
- [ElephantSQL](https://www.elephantsql.com/  "ElephantSQL") to store the database until 31.01.24. On that date, *Code Institute Postgres Database* was used. 
...
- *test_form_is_valid*: This test checks if the form is valid when all required fields are provided. It passes if is_valid() returns True, confirming that the form accepts the correct input.
...
- the test failed becaues it expects a 200 OK response, but since the view redirects (return redirect('book_detail', bibliography_id=bibliography_id)), it returns 302 Redirect instead. The refactored test asserts a new condition, resulting in the successful test.
...
In this document, the issues that are not connected to the specificity of the Django syntax are addressed: [Stray end tag](./docs/images/Stray%20end%20tag.png); [Duplicate attribute](./docs/images/detail%20Duplicate%20Attribute.png); 
...
**JavaScript:**
- [JSLint](docs/images/js%20validation%201.png) doesn't support all ESJ features, hence minor error reports. When possible, the raised issues were addressed, such as the [too long](docs/images/js%20validation%202.png) text in the comments. The [Esprima](https://esprima.org/demo/validate.htm "Esprima JS Validation Platform") software was also used for the validation.  
...
- the file "book/models.py" was checked in [Pythonium](https://pythonium.net/linter) validator, which gave this [report](docs/images/pythonium%20models.png).
- the file "book/test_forms.py" was checked in [Pythonium](https://pythonium.net/linter) validator, which gave this [report](docs/images/pythonium%20test%20review%20form.png).
- the file "book/TestReviewViews.py" was checked in [Pythonium](https://pythonium.net/linter) validator, which gave this [report](docs/images/pythonium%20TestReviewViews.png).
- the file "settings.py" was checked in [Pythonium](https://pythonium.net/linter) validator, which gave this [report](docs/images/pythonium%20settings.png).
...
### Further improvement
 - Using slugs (book keywords) instead of the object's ID is possible to improve the site's ranking in search engines.
